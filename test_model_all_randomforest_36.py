...
for depth_num in range(1, depths_to_test + 1):
	with open(f"./random_forest_{depth_to_test}.pkl", "rb") as f:
		rf = pickle.load(f)
...
plt.title("Depth of Random Forest vs Accurary")
plt.xlabel("Depth of Random Forest")
plt.ylabel("Accurary Percent")
plt.plot(np.arange(1, depths_to_test), accuracy_list, color ="blue")
plt.show()
"""


with open(f"./random_forest_{max_accuracy_depth}.pkl", "rb") as f:
	rf = pickle.load(f)
...
plt.title("Feature Importances (Top 10) For Random Forest")
plt.xlabel("Mean Positive SHAP Value (Average Impact on Model)")
plt.ylabel("Feature")
plt.show()
plt.savefig('random_forest_factor_importance.png')
